...
function getDirections(piece: Piece, allowBackward: boolean = false): [number, number][] {
  if (isKing(piece) || allowBackward) return KING_DIRECTIONS;
...
export function getSimpleMoves(board: Board, row: number, col: number): MoveAction[] {
  ...
  for (const [dr, dc] of dirs) {
    ...
    actions.push({ steps: [{ from: [row, col], to: [nr, nc], captured: null }] });
  }
  return actions;
}

export function getCaptures(board: Board, row: number, col: number, allowBackward: boolean = false): MoveStep[] {
  const piece = board[row][col];
  if (!piece) return [];
  const captures: MoveStep[] = [];
  const dirs = getDirections(piece, allowBackward);
  const color = getPieceColor(piece);
  for (const [dr, dc] of dirs) {
    ...
    if (midPiece && isEnemy(midPiece, color) && landPiece === null) {
      captures.push({ from: [row, col], to: [lr, lc], captured: [mr, mc] });
    }
  }
  return captures;
}

export function getCaptureActions(board: Board, row: number, col: number): MoveAction[] {
  const initialCaptures = getCaptures(board, row, col, false);
  if (initialCaptures.length === 0) return [];
  const actions: MoveAction[] = [];
  for (const step of initialCaptures) {
    const newBoard = applyMove(board, step);
    const piece = board[row][col];
    const afterCapture = piece && !isKing(piece);
    const chains = getCaptureChains(newBoard, step.to[0], step.to[1], afterCapture);
    if (chains.length === 0) {
...
function getCaptureChains(board: Board, row: number, col: number, allowBackward: boolean): MoveStep[][] {
  const more = getCaptures(board, row, col, allowBackward);
  if (more.length === 0) return [[]];
  const chains: MoveStep[][] = [];
  for (const step of more) {
    const newBoard = applyMove(board, step);
    const subChains = getCaptureChains(newBoard, step.to[0], step.to[1], true);
...
export function getAllActions(board: Board, color: Color): MoveAction[] {
  ...
  for (let row = 0; row < 8; row++) {
    for (let col = 0; col < 8; col++) {
      ...
      if (captures.length > 0) actions.push(...captures);
    }
  }
...
export function getActionsForPiece(board: Board, row: number, col: number): MoveAction[] {
  const piece = board[row][col];
  if (!piece) return [];
  const captures = getCaptureActions(board, row, col);
