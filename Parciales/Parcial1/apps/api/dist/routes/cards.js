...
cards.get('/', async (c) => {
    ...
    if (parsed.game)
        filter.game = parsed.game;
    if (parsed.condition)
        filter.condition = parsed.condition;
...
cards.get('/:id', async (c) => {
    ...
    const card = await Card.findByIdAndUpdate(id, { $inc: { views: 1 } }, { new: true }).populate('sellerId', 'username avatar rating reviewCount');
    if (!card) {
...
cards.patch('/:id', async (c) => {
    ...
    const card = await Card.findOneAndUpdate({ _id: id, sellerId: user._id }, { ...parsed, updatedAt: new Date() }, { new: true });
    if (!card) {
