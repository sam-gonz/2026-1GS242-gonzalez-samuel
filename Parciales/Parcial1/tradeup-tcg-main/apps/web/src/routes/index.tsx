...
const FEATURES = [
  {
    icon: '🔄',
    title: 'Intercambios C2C',
    desc: 'Ofrece tus cartas, propone intercambios directos o mixtos con otros coleccionistas.',
  },
  {
    icon: '🏪',
    title: 'Tienda Oficial',
    desc: 'Cartas selladas y singles gradeados directamente de TradeUp con garantía de autenticidad.',
  },
  {
    icon: '🔒',
    title: 'Pagos Seguros',
    desc: 'Stripe Connect mantiene el dinero en hold hasta que ambas partes confirmen la transacción.',
  },
  {
    icon: '⭐',
    title: 'Sistema de Reputación',
    desc: 'Reviews verificadas por transacción real. Sabes exactamente con quién estás tratando.',
  },
  {
    icon: '🃏',
    title: 'Catálogo TCG',
    desc: 'Pokémon, Yu-Gi-Oh!, One Piece, Dragon Ball, MTG y más. Búsqueda instantánea.',
  },
  {
    icon: '📱',
    title: 'Diseñado para Coleccionistas',
    desc: 'Interface oscura, rápida y sin distracciones. Tu colección, tu ritmo.',
  },
]

const GAMES = [
  { name: 'Pokémon', emoji: '⚡', color: 'from-yellow-500/20 to-transparent' },
  { name: 'Yu-Gi-Oh!', emoji: '👁️', color: 'from-purple-500/20 to-transparent' },
  { name: 'One Piece', emoji: '🏴‍☠️', color: 'from-red-500/20 to-transparent' },
  { name: 'Dragon Ball', emoji: '🔮', color: 'from-orange-500/20 to-transparent' },
  { name: 'MTG', emoji: '🪄', color: 'from-blue-500/20 to-transparent' },
]
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          >
            {GAMES.map((g) => (
              <span
                key={g.name}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[var(--color-surface-2)] border border-[var(--color-border)] text-sm text-[var(--color-muted)] hover:text-white hover:border-[var(--color-brand)]/50 transition-colors cursor-default"
              >
                <span>{g.emoji}</span>
                {g.name}
              </span>
      ...
      <section className="max-w-7xl mx-auto px-4 py-24">
        <div className="text-center mb-16">
          <h2 className="font-display text-3xl md:text-4xl font-bold text-white mb-4">
            Todo lo que necesitas para coleccionar
          </h2>
          <p className="text-[var(--color-muted)] max-w-xl mx-auto">
            Diseñado por coleccionistas, para coleccionistas.
          </p>
        </div>
      ...
      <section className="max-w-3xl mx-auto px-4 py-24 text-center">
        <div className="p-10 rounded-2xl bg-gradient-to-br from-[var(--color-brand)]/20 to-[var(--color-surface-2)] border border-[var(--color-brand)]/30">
          ...
          <p className="text-[var(--color-muted)] mb-8">
            Regístrate gratis y empieza a intercambiar en minutos.
          </p>
          <Link
            to="/marketplace"
