      ...
      <div className="victory-screen anim-fade-in">
        {didWin ? (
          <>
            ...
            <p>Derrotaste a {oppPlayer?.name}.</p>
          </>
        ) : (
          <>
            <h1 className="defeat-title">DERROTA</h1>
            <p>{battle.winnerPlayerId} gano la batalla.</p>
          </>
        )}
    ...
    <div className="battle-screen">
      ...
      <div className="battle-arena">
        ...
        <div className={`pokemon-slot ${oppActive?.currentHp <= 0 ? 'pokemon-slot--fainted' : ''}`}>
          ...
          <img src={oppActive?.spriteUrl} alt={oppActive?.name}
          ...
          {oppActive?.status && (
            <span className={`status-badge status-${oppActive.status.name}`}>
              {oppActive.status.name.toUpperCase()} ({oppActive.status.remainingTurns}T)
            </span>
          )}
        ...
        <div className={`pokemon-slot pokemon-slot--active ${myActive?.currentHp <= 0 ? 'pokemon-slot--fainted' : ''}`}>
          <span style={{ fontFamily: 'var(--font-display)', fontSize: '7px', color: 'var(--accent)', alignSelf: 'flex-start' }}>TU POKEMON</span>
          <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', justifyContent: 'center' }}>
            {myActive?.types?.map((t: string) => <span key={t} className={`type-badge type-${t}`}>{t}</span>)}
          </div>
          <img src={myActive?.spriteUrl} alt={myActive?.name}
          ...
          {myActive?.status && (
            <span className={`status-badge status-${myActive.status.name}`}>
              {myActive.status.name.toUpperCase()} ({myActive.status.remainingTurns}T)
            </span>
          )}
            ...
            {(myPlayer?.team ?? []).map((p: any) => {
              ...
              return (
                ...
                >
                  ...
                  <span style={{ marginLeft: 'auto', fontFamily: 'var(--font-body)', fontSize: '11px', color: 'var(--text-muted)' }}>
                    {p.currentHp}/{p.maxHp}
                  </span>
